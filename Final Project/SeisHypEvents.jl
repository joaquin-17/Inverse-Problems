...
function SeisHypEvents(; ot::Real=0.0, dt::Real=0.004, nt::Int=301,
                        ...
                        f0::Vector{T5}=[20.0],f=[2.0, 10.0, 40.0, 60.0]) where {T1<:Real, T2<:Real, T3<:Real, T4<:Real, T5<:Real}
    ...
    for ievent = 1:nevents
        ...
        elseif wavelet == "ormsby"

            wav = Ormsby(dt=dt, f=f)
            nwav = length(wav)
            wav = cat(wav, zeros(nf-nwav),dims=1)
            Wav = fft(wav)
            delay = dt*(floor(Int, nwav/2))
            ...
            error("Other wavelets apart from Ricker and Ormsby are not implemented yet")
        end
        for iw = 1:nw
            w = (iw - 1)*dw
